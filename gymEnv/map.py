...
class Map:
    def __init__(self, width, height, view):
        ...
        self.w = width
        self.h = height
        self.m = np.zeros((self.maxh, self.maxw))
        self.unknownMap = np.ones((self.maxh, self.maxw))
        # self.m = [[0] * width for y in range(height + view + 1)]
        self.playerMap = np.zeros((self.maxh, self.maxw))
        self.enemyMap = np.zeros((self.maxh, self.maxw))
        self.maxy = 0
...
def main():                                
    ...
    while True:
        ...
        for j in range(2):
            ...
            ps.append(tuple(p))
        self.playerMap = np.zeros()
        self.playerMap[ps[0][0][0], ps[0][0][1]] = 1
        self.
