...
class SamuraiGym2(gym.Env):
    metadata = {'render.modes': ['human', 'viewer', 'human+viewer']}
    # MAX_STEPS = 100

    def __init__(self):
        super().__init__()
        self.mapName = './samples/course07middle.smrjky'
        self.rewardFunc = self.get_reward7
        ...
        self.reward_range = [-5000., 10000.]
        self.logFile = 0
        # course01はlength+visionが110あるので避ける
        with open(self.mapName, 'r') as mapFile:
    ...
    def _reset(self):
        ...
        self.step_num = 0
        # stateに参照を渡す前にmapの設定を終わらせておかないと、stateからの参照がおかしくなる?
        # mapでplayer=player.newをするとstate側からのplayer参照が前のままになってしまう
        # mapでplayer.newをしてから参照をstate.newに渡す
        self.map.reset()
    ...
    def _step(self, action):
        ...
        # -5 <= speed[0] <= 5,
        # -2 <= speed[1] <= 5
        # を外れる動きをした時に、各軸の加速度0へ
        # rewardをマイナスして動きを覚えさせる
        speed = self.map.player.speed
        reward = 0
        if speed[0] > 5 and acc[0] > 0:
            acc[0] = 0
            reward = -1
        elif speed[0] < -5 and acc[0] < 0:
            acc[0] = 0
            reward = -1
        if speed[1] > 5 and acc[1] > 0:
            acc[1] = 0
            reward = -1
        elif speed[1] < -2 and acc[1] < 0:
            acc[1] = 0
            reward = -1
        ...
        assert observation.shape == self.observation_space.shape, "Not equal observation.shape"
        reward += float(self.rewardFunc())
    ...
    def _render(self, mode='human', close=False):
        ...
        def viewer():
            if self.isDone:
                ...
                logFileName = "gamelog" + str(self.logFile) + ".json"
    ...
    def get_reward3(self):
        if self.isDone:
            ...
            return float(self.map.player.speed[1] * 1)
    ...
    def get_reward7(self):
        if self.isClashed:
            return -1
        moved_percent = self.map.player.speed[1] / self.state.goalLength * 100.0
        # 最後にrewardを調節するのはrewardの予測が難しくなる
        # 毎ターン-1する
        # moved_percentのmaxが100
        # マイナスもありうる
        rwd = moved_percent - 1.
        return rwd
