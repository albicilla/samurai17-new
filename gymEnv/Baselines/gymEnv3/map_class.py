    ...
    def move_jockey(self, acc):
        ...
        if self.map.has_collision(self.pos, next_p):
            ...
            if next_p[1] >= self.map.h:
                return True, isClashed
        return False, isClashed
...
class Map:
    ...
    def reset(self):
        self.player = Jockey(self, self.smrjky["x0"], 0, 0, 0, self.maxVision)
        self.enemy = Jockey(self, self.smrjky["x1"], 0, 0, 0, self.maxVision)
