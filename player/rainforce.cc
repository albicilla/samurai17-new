...
PlayerState get_action(RaceState &rs, const Course &course){

    //速度を9種類全てループ
    for (int cay = 1; cay != -2; cay--) {
        for (int cax = -1; cax != 2; cax++) {
            PlayerState next_state(nextPos, nextVelo);

            //100ばい
            int epsilon = 50*pow(0.9,episode);
            //0~1 ->0~100
            random_device rnd;     // 非決定的な乱数生成器
            int togo=rnd()%101;
            if(epsilon <= togo){
                //次に遷移できる状態で取れる行動の中で最大の評価値の行動を選ぶ
                double temp =-1;
                rep()
                next_action =　q_table[next_state.velocity.x][next_state.velocity.y][next_state.position.x][][][];
            }else{
                //ランダムに次の行動を決定する
                next_action =
            }
            ...
            double alpha = 0.5;
            double gamma = 0.99;
            q_table[rs.velocity.x][rs.velocity.y][rs.position.x][][][]=(1-alpha)*q_table[][]
        }
    }
