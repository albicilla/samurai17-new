...
double calcCost(Candidate *now, Point nextPos, RaceState &rs,int depth,Point nextVelo) {
    ...
    //cerr<<abs((nextVelo.y)/2) <<abs((nextVelo.x)/2)<<(vision+1)/2<<endl;
    if( ( abs((nextVelo.y)-1) + abs((nextVelo.x)) )>(vision+1)/2)ret-=50;

    //if(abs(nextVelo.x))ret-=0.01;
    ...
    //中心に寄ってる方が若干良い
    ret+=sigmoid(1,abs(nextPos.x-CourseWidth/2.0))/100;
    return ret;
}
...
bool target_insite(RaceState &rs){
    ...
    for(int j= rs.position.y+vision;j>(rs.position.y-vision);j--){
        for(int i=0;i<CourseWidth;i++){
            if(course[i][j]==1){
                total_point++;
                if(j>rs.position.y)total_obp++;
            }else{
                total_point2++;
            }
        }
    ...
    if(total_point > total_point2 /2 )return true;
    else return false;
}
ofstream otemp("/Users/albicilla/programming/samurai/samurai17/test.txt",ios::out);
ofstream outputfile("/Users/albicilla/programming/samurai/samurai17/test.txt",ios::app);
IntVec play(RaceState &rs){
    cerr<<"now.step="<<rs.step<<endl;

    //コースの形状から何かする
    is_dig=false;
    is_dig=target_insite(rs);
    if(is_dig)OVERCLOCK=5;

    for(int tate=rs.position.y+vision+predi;tate>=rs.position.y-vision;tate--){
        {
            rep(yoko,CourseWidth){
...
void prediction(RaceState &rs){
    for(int i=vision+1;i<=vision+predi;i++){
        rep(j,care){
            course[rs.position.y+i+care].push_back(1);
        }
        for(int j=0;j<CourseWidth;j++){

            if(COURSE(rs.position.y+i-1,j)==1 && COURSE(rs.position.y+i-2,j)==1&& COURSE(rs.position.y+i-3,j)==1){
                COURSE(rs.position.y+i,j)=1;
            }
...
int main(){
    ...
    while(1){
        ...
        //消費timeが大きすぎたならdepthを下げる
        if(dTime-rs.timeLeft>300){
            //流石に7よりは下げたくない
            if(MAX_DEPTH>7)MAX_DEPTH-=1;
        }

        dTime=rs.timeLeft;

        for(int i=-vision;i<=vision+predi;i++){
