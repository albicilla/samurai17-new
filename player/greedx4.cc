...
int CCW(Point p0,Point p1,Point p2){
    ...
    if(p2==p0||p2==p1)return ONLINE_FRONT;
...
double calcCostInher(RaceState &rs,int depth,int dominance,Point nextoppPos,Point nextPos){
    ...
    if(CCW(rs.oppPosition,nextoppPos,nextPos)==ON_SEGMENT && dominance==1 && depth==0){
        ret=2;
    }

    if(CCW(rs.oppPosition,nextoppPos,nextPos)==ON_SEGMENT && dominance==2 && depth==0){
        ret=-2;
    }
    return ret;
}
...
double calcCost(Candidate *now, Point nextPos, RaceState &rs,const Course &course,int depth) {
    ...
    if(now->state.velocity.y>course.vision-2)ret-=10;
    //視界の外への移動に対する罰則
    ...
    while (!candidates.empty()) {
        ...
        for (int cay = 1; cay != -2; cay--) {
            for (int cax = -1; cax != 2; cax++) {
                ...
                if (!course.obstacled(now->state.position, nextPos) ) {
                    ...
                    if (nextPos.y >= course.length) {
                        ...
                        if (nextPos.y < course.length && nextPos.x >= 0 &&
                            ...
                            //評価値の計算
                            nextCand->cost = calcCost(now, nextPos, rs,course,depth)+special_cost;
                ...
                else if(((flag_general)||(same_pos)) && !(flag_use_greedy2)){
                    //足しておいたのを打ち消す
                    ...
                    if (reached.count(next) == 0 && abs(nextVelo.x)+abs(nextVelo.y)>2) { //そこにたどり着くのが最初の候補であれば
                        //コースをはみ出していなければ
