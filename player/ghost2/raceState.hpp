...
struct Course {
    ...
    bool target_insite(const Course &course,int a,int b) const;
    double hyouzan(const Course &course,int a,int b,Point nextPos) const;
    void predict(Course &course,int a,int b);
    bool is_obs(const Course &course,int a,int b)const;
    void back(const Course &course,int a,int b)const;
    Course(istream &in);
};
