...
bool is_dig=false;
//穴掘り型か
bool Course::target_insite(const Course &course,int rsx,int rsy)const{
    ...
    //前方の障害物
    int total_obp=0;
//    for(int j=rsy+course.vision;j>(rsy-course.vision);j--){
//        for(int i=0;i<course.width;i++){
//            if(obstacle[j][i]==ObstState::OBSTACLE) {
//                int point =0;
//                for(int idx_x=0;idx_x<4;idx_x++){
//                    for(int idx_y=0;idx_y<4;idx_y++){
//                        int look_x=idx_x+i;
//                        int look_y=idx_y+j;
//                        if(look_x<0 || look_x>=course.width || look_y<0 || look_y>=course.length)continue;
//                        else{
//                            if(obstacle[j][i]==ObstState::OBSTACLE)point++;
//                        }
//                    }
//                }
//                if(point>3)total_point++;
//            }else{
//                // cerr << course.obstacle[rs.position.y][rs.position.x] <<endl;
//                // cerr<<"0";
//            }
//        }
//        // cerr<<endl;
...
double calcCost(Candidate *now, Point nextPos, RaceState &rs,const Course &course,int depth,Point nextVelo) {
    ...
    if( ( abs((nextVelo.y)-1) + abs((nextVelo.x)) )>(course.vision+1)/2)ret-=50;

    if(nextVelo.x<=0 &&((nextVelo.x)+nextPos.x < 0+nextVelo.x)){
        ret-=10;
    }

    if(nextVelo.x >0 && ((nextVelo.x)+nextPos.x)>course.width -nextVelo.x){
        ret-=10;
    }
    ...
    while (!candidates.empty()) {
        ...
        for (int cay = -1; cay <= 1; cay++) {
            for (int cax = -1; cax <=1; cax++) {
                ...
                if (!course.obstacled(now->state.position, nextPos) && (nextPos.y<=((ini_y+course.vision)+OVERCLOCK))  ) {
                    ...
                    //次の候補でゴールが可能ならば
                    if (nextPos.y >= course.length && nextPos.y<=(ini_y+course.vision) ){
                        ...
                        if (nextPos.y < course.length && nextPos.x >= 0 &&
                            ...
                            //評価値の計算
                            nextCand->cost = calcCost(now, nextPos, rs,course,depth,nextVelo)+nextCand->special_cost;
...
IntVec play(RaceState &rs, const Course &course) {
    ...
    int rs_x=rs.position.x;
    int rs_y=rs.position.y;
    //コースの形状から何かする
    is_dig=false;
    is_dig=course.target_insite(course,rs_x,rs_y);
...
//    }
    ...
    //cerr  << "t" << endl;
    //initialはプレイヤーの状態 自機の位置、速度 スペシャルコスト
